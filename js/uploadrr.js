    ...
    $.fn.Uploadrr = function (options) {
        ...
        var internal = {
            ...
            render: function (element) {
                ...
                if (this.settings.simpleFile == true) this.fileField.css({
                    'opacity': 1
                });
                else {
                    this.fakeDiv = $('<div/>', {
                    ...
                    if ($.browser.msie) {
                        ...
                        this.ftext = $('<input/>', {
                            'type': 'text',
                            'class': 'text bev',
                            'onClick': 'document.getElementById(\'uploadrealid\').click()'
                        });
                    }
                    this.fakeDiv.append(this.ftext);
                    if ($.browser.msie) {
                        this.fakeDiv.append($('<input/>', {
                            'type': 'button',
                            'class': 'button bev xx',
                            'value': langArray[0]
                        }));
                ...
                this.upInfoDiv.append($('<img/>', {
                    'src': this.settings.progressGIF,
                    'id': 'prImg'
                }));
                ...
                this.info = $('<div/>', {
                    'class': 'info'
                }).html(langArray[1] + (this.DragDropSupported ? langArray[2] : '.') + (typeof this.settings.allowedExtensions == 'object' ? '<br><br>' + langArray[3] + this.settings.allowedExtensions.join(', ') : ''));
                this.fileListDiv.append(this.info);
                fform.append(this.fileListDiv);
                this.submit = $('<input/>', {
                    'type': 'button',
                    'value': langArray[4],
                    'class': $.browser.msie ? 'ib' : 'button bev'
                }).css({
                    'width': 'auto',
                    'position': 'relative',
                    'padding': '6px 20px'
            ...
            checkExt: function (fileName) {
                ...
                if (typeof this.settings.allowedExtensions == 'object') {
                    if (this.settings.allowedExtensions.indexOf(fileExt) > -1) return true;
                    else alert(fileName + langArray[5] + this.settings.allowedExtensions.join(', '));
                } else return true;
            },
            addFile: function (e) {
                if (e.files) caller = e;
                ...
                appendFile = function (file) {
                    ...
                    delBtn.bind('click', {
                        ...
                        if (event.data._this.ftext) event.data._this.ftext.val(event.data.fname + langArray[7]);
                    });
                }
                if ((caller.files) && (new XMLHttpRequest().upload != null)) {
                    ...
                    for (i = 0; i < caller.files.length; i++) {
                        ...
                        if (this.checkExt(file.name)) {
                            if (file.size < this.settings.maxFileSize || this.settings.maxFileSize == -1) {
                                ...
                                ct = l == 1 ? langArray[8] : langArray[9];
                                this.ftext.val(l + ct + langArray[20]);
                            } else alert(langArray[10] + this.settings.maxFileSize + ' bytes.');
                        };
                    }
                } else {
                    if (this.checkExt(caller.value)) {
                        ...
                        this.fileField = $('<input/>', {
                            ...
                            'type': 'file',
                            'multiple': 'true'
                        }).bind('change', $.proxy(this.addFile, this)).css({
                            'opacity': 1
                        });
            ...
            sendFiles: function () {
                ...
                this.submit.hide(350);
                this.fakeDiv.hide(700);
                this.realDiv.hide(700);
                this.fileListDiv.animate({
                    marginTop: "-=40px"
                }, 950);
                if (!(this.upInfoDiv.is(":visible"))) {
                    this.upInfoDiv.slideDown('fast');
                }
                if (this.files.modern) {
                    ...
                    $(this.upFrame).bind('load', $.proxy(function () {
                        ...
                        try {
                            this.reply = $.parseJSON(this.reply);
                        } catch (err) {
                            this.uploadError('jsonError');
                        };
                        if (this.reply.success) {
                            if (this.nform) this.nform.remove();
                            $(this.fileListDiv.children('.fileItem')[this.current].children[1]).css({
                                'width': "100%"
                            });
                            fname = this.files.items[this.current].value.split('\\');
                            fname = fname[fname.length - 1];
                            this.current++;
                            if (this.files.items[this.current]) {
                                this.uploadStandart(this.files.items[this.current]);
                            } else {
                                this.allDone();
                                if ($.browser.msie) {
            ...
            uploadProgress: function (evt) {
                ...
                this.filePro.html(langArray[12] + percent + "% " + loaded + "KB/" + total + "KB");
                this.totalPro.html(langArray[13] + (this.current + 1) + '/' + this.fileCount + langArray[14] + ((this.totalLoaded + evt.loaded) / 1024).toFixed() + '/' + this.totalSize + 'KB');
                $(this.fileListDiv.children('.fileItem')[this.current].children[1]).css({
                    'width': percent + "%"
                });
            },
            uploadFinished: function (evt) {
                ...
                this.filePro.html(langArray[12] + ' 100% ' + total + 'KB/' + total + 'KB');
                this.totalPro.html(langArray[13] + (this.current + 1) + '/' + this.fileCount + langArray[14] + (this.totalLoaded / 1024).toFixed() + '/' + this.totalSize + 'KB')
                $(this.fileListDiv.children('.fileItem')[this.current].children[1]).css({
                    'width': "100%"
                });
            },
            allDone: function () {
                this.upInfoDiv.html('<p>' + langArray[15] + '</p>').addClass('result').css({
                    backgroundColor: "#CDEB8B"
                });
                this.settings.onComplete();
            },
            uploadError: function (errorType) {
                this.upInfoDiv.html('').addClass('result').css({
                ...
                switch (errorType) {
                case 'statusError':
                    response = '<b>Bad Response: ' + this.xhr.status + ' ' + this.xhr.statusText + '</b>';
                    break;
                case 'customError':
                    response = '<b>Server Error: ' + this.reply.details + '</b>';
                    break;
                case 'jsonError':
                    response = '<b>' + langArray[16] + '</b>';
                    break;
                case 'fileMatchError':
                    response = '<b>' + langArray[17] + '</b>';
                    break;
                default:
                    response = '<b>' + langArray[18] + '</b>';
                    break;
                }
                    ...
                    this.xhr.onreadystatechange = $.proxy(function () {
                        if (this.xhr.readyState == 4) {
                            if (this.xhr.status < 400 && this.xhr.status >= 200) {
                                try {
                                    this.reply = $.parseJSON(this.xhr.responseText);
                                } catch (err) {
                                    this.uploadError('jsonError');
                                };
                                if (this.reply.success) {
                                    this.current++;
                                    if (this.files.items[this.current]) {
                                        this.uploadModern(this.files.items[this.current]);
                                    } else this.allDone();
                                } else this.uploadError('customError');
            ...
            uploadStandart: function (file) {
                this.filePro.html(langArray[19] + file.value);
