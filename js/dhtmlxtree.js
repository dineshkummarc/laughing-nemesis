...
dhtmlXTreeObject.prototype._parse = function (p, parentId, level, start) {
    ...
    if ((temp.childsCount) && (!start) && (!this._edsbps) && (!temp._has_top)) var preNode = temp.childNodes[temp.childsCount - 1];
    else var preNode = 0;
    ...
    if (!level) {
        ...
        if (this.waitUpdateXML) {
            ...
            if (temp.childNodes[i]._dmark) this.deleteItem(temp.childNodes[i].id);
        }
...
dhtmlXTreeObject.prototype._xcloseAll = function (itemObject) {
    ...
    if (itemObject.childNodes[i].childsCount) this._xcloseAll(itemObject.childNodes[i]);
};
/**
...
dhtmlXTreeObject.prototype.onRowSelect = function (e, htmlObject, mode) {
    ...
    if (!mode) {
        if ((e) && (e.button == 2)) that.callEvent("onRightClick", [obj.id, e]);
...
dhtmlXTreeObject.prototype._createItem = function (acheck, itemObject, mode) {
    ...
    td11.style.width = "0px";
	td11.style.display = "none";

    var td12 = document.createElement('span');
    ...
    img.style.margin = 0;
    if (this.timgen) {
        td12.style.width = img.style.width = "0px";
    ...
    itemObject.span.style.paddingRight = "1px";
    ...
    if (this.checkEvent && this.checkEvent("onRightClick")) tr.oncontextmenu = Function("e", "this.childNodes[0].parentObject.treeNod.callEvent('onRightClick',[this.childNodes[0].parentObject.id,(e||window.event)]); return false;");
...
dhtmlXTreeObject.prototype._setCheck = function (sNode, state) {
    ...
    if (((sNode.parentObject._r_logic) || (this._frbtr)) && (state)) if (this._frbtrs) {
        ...
        this._frbtrL = sNode;
    } else for (var i = 0; i < sNode.parentObject.childsCount; i++)
...
dhtmlXTreeObject.prototype._setSubChecked = function (state, sNode) {
    state = convertStringToBoolean(state);
    if (!sNode) return;
    if (((sNode.parentObject._r_logic) || (this._frbtr)) && (state)) for (var i = 0; i < sNode.parentObject.childsCount; i++)
...
dhtmlXTreeObject.prototype._createDragNode = function (htmlObject, e) {
    ...
    var text = new Array();
    if (this._itim_dg) for (var i = 0; i < this._selected.length; i++)
...
dhtmlXTreeObject.prototype._focusNode = function (item) {
    ...
    if ((z > (this.allTree.scrollTop + this.allTree.offsetHeight - 30)) || (z < this.allTree.scrollTop)) this.allTree.scrollTop = z;
};
...
dhtmlXTreeObject.prototype._dragIn = function (htmlObject, shtmlObject, x, y) {
    ...
    if (!this.callEvent("onDragIn", [fobj.id, tobj ? tobj.id : null, fobj.treeNod, this])) return 0;
...
dhtmlXTreeObject.prototype.setItemStyle = function (itemId, style_string) {
    ...
    else temp.span.style.cssText += (";" + style_string);
}
